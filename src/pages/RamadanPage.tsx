...
function getRamadanImsakia(location: { lat: number; lon: number }, year: number, month: number): DayEntry[] {
  const entries: DayEntry[] = [];
  const coords = new adhan.Coordinates(location.lat, location.lon);
  const params = (adhan.CalculationMethod as any).Egyptian();

  for (let d = 1; d <= 30; d++) {
    // Approximate Ramadan day in Gregorian
    const approxDate = new Date(year, month - 1, d);
    const dc = new adhan.DateComponents(approxDate.getFullYear(), approxDate.getMonth() + 1, approxDate.getDate());
    const times = new adhan.PrayerTimes(coords, dc, params);

    entries.push({
      day: d,
      date: approxDate.toLocaleDateString('ar-EG', { day: 'numeric', month: 'short' }),
...
export default function RamadanPage() {
  ...
  useEffect(() => {
    if (location) {
      // Get approximate Gregorian date of current/next Ramadan
      const now = new Date();
      const entries = getRamadanImsakia(location, now.getFullYear(), now.getMonth() + 1);
      setImsakia(entries);
    }
  }, [location]);

  const todayEntry = inRamadan ? imsakia[todayHijri.day - 1] : null;
    ...
    <div>
        ...
        {inRamadan ? (
          ...
          <>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginTop: 6 }}>
              الشهر الهجري الحالي: {todayHijri.monthName}
            </p>
          </>
        )}
      </div>
          ...
          {imsakia.length > 0 && (
            <div>
              ...
              {imsakia.map(entry => (
                ...
                >
                  <span style={{ color: entry.day === ramadanDay ? 'var(--gold)' : 'var(--text-secondary)', fontWeight: entry.day === ramadanDay ? 700 : 400 }}>
                    {entry.day}
                  </span>
      ...
      {tab === 'info' && (
        <div style={{ padding: '12px 16px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            { icon: '📿', title: 'فضل رمضان', text: 'شَهْرُ رَمَضَانَ الَّذِي أُنزِلَ فِيهِ الْقُرْآنُ هُدًى لِّلنَّاسِ وَبَيِّنَاتٍ مِّنَ الْهُدَىٰ وَالْفُرْقَانِ (البقرة: 185)' },
            { icon: '🌙', title: 'ليلة القدر', text: 'خير من ألف شهر - ابحث عنها في الأوتار من العشر الأواخر' },
            { icon: '🤲', title: 'الاعتكاف', text: 'سنة في العشر الأواخر من رمضان التزاماً بسنة النبي ﷺ' },
            { icon: '💝', title: 'الزكاة والصدقة', text: 'أفضل الصدقة صدقة في رمضان - تضاعف الحسنات' },
            { icon: '📖', title: 'ختم القرآن', text: 'استهدف ختم القرآن مرة على الأقل في رمضان - جزء يومياً' },
          ].map((item, i) => (
